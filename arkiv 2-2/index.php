...
<table border="1">
    ...
    if ($result->num_rows > 0) {
        ...
        while($row = $result->fetch_assoc()) {
            ...
            echo "<td><a href='#' onclick='showPopup(\"" . $row["kontaktpersonID"] . "\", \"" . $row["fornavn"] . "\", \"" . $row["etternavn"] . "\", \"" . $row["epost"] . "\", \"" . $row["telefon"] . "\", \"" . $row["stilling"] . "\")'>Rediger kontaktperson</a> | <a href='#' onclick='showCompanyPopup(\"" . $row["bedrift_id"] . "\", \"" . $row["bedrift_navn"] . "\", \"" . $row["fornavn"] . "\", \"" . $row["etternavn"] . "\")'>Rediger bedrift</a> | <a href='delete_contact.php?contact_id=" . $row["kontaktpersonID"] . "' onclick='return confirm(\"Er du sikker på at du vil slette denne kontaktpersonen?\")'>Slett kontaktperson</a></td>";
            echo "</tr>";
        }
    } else {
...
<!-- Legg til knapp -->
<button id="legg-til-knapp" onclick="location.href='add_company.php'">Legg til bedrift</button>
